...
# Resolve this script's own directory so compose files (now under docker/) are
# referenced by absolute path regardless of the caller's working directory.
...
if [[ "$enable_gpu" == true ]]; then
  ...
  compose_files+=("-f" "$SCRIPT_DIR/docker-compose.gpu.yaml")
fi

if [[ "$enable_api" == true ]]; then
  export OLLAMA_WEBAPI_PORT="$api_port"
  compose_files+=("-f" "$SCRIPT_DIR/docker-compose.api.yaml")
fi

if [[ -n "$data_dir" ]]; then
  export OLLAMA_DATA_DIR="$data_dir"
  compose_files+=("-f" "$SCRIPT_DIR/docker-compose.data.yaml")
fi

if [[ "$enable_playwright" == true ]]; then
  compose_files+=("-f" "$SCRIPT_DIR/docker-compose.playwright.yaml")
fi
